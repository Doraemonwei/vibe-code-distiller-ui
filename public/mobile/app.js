...
function connectWebSocket() {
    // Use Socket.IO instead of raw WebSocket for mobile chat
    if (typeof io !== 'undefined') {
        // Pass project context via connection query
        const projectName = getCurrentProjectFromURL();
        console.log('Connecting WebSocket with project:', projectName);
    ...
    ws.on('connect', async () => {
        ...
        // Get project context from URL (no more localStorage race conditions!)
        updateProjectDisplayFromURL();
        
        // Load settings and conversation list (provider will use URL project context)
        ws.emit('mobile:getSettings');
        setTimeout(() => {
            ws.emit('mobile:getConversationList');
        }, 100);
    });
    
    ws.on('mobile:message', (message) => {
...
function handleMessage(message) {
    switch (message.type) {
        ...
        case 'projectSwitched':
            ...
            if (currentProjectEl) {
                currentProjectEl.textContent = message.data.projectName;
            }
...
function addToolResultMessage(data) {
    // Tool results are internal system communication - don't display them in chat
    // They're already processed and Claude will respond with user-facing messages
    console.log('Tool result received (not displayed):', data);
    return;
}
...
function loadConversation(sessionId) {
    ...
    ws.emit('mobile:loadConversation', { sessionId });
...
function changeModel(model) {
    ...
    ws.emit('mobile:selectModel', { model });
...
function respondToPermission(id, approved, alwaysAllow) {
    // Send response
    ws.emit('mobile:permissionResponse', {
        id,
        approved,
        alwaysAllow
    });
...
function showFilePicker() {
    ...
    ws.emit('mobile:getWorkspaceFiles', { searchTerm: '' });
...
function getCurrentProjectFromURL() {
    ...
    const projectMatch = urlPath.match(/\/mobile\/project\/([^/?]+)/);
    if (projectMatch) {
        return decodeURIComponent(projectMatch[1]);
    }
    // Fallback for root mobile URL
    return 'Current Project';
}

// Simple function to update project display from URL
function updateProjectDisplayFromURL() {
    const projectName = getCurrentProjectFromURL();
    const currentProjectEl = document.getElementById('currentProject');
    if (currentProjectEl) {
        currentProjectEl.textContent = projectName;
    }
}

function updateCurrentProjectDisplay(data) {
    const currentProjectEl = document.getElementById('currentProject');
    if (currentProjectEl) {
        const current = data.projects.find(p => p.isCurrent);
        currentProjectEl.textContent = current ? current.name : 'No Project';
    }
}
...
async function loadProjects() {
    try {
        ...
        // Update header with current project
        updateCurrentProjectDisplay(data);
    } catch (error) {
...
async function switchProject(projectPath) {
    try {
        ...
        // Redirect to the project-specific URL (no more localStorage!)
        if (data.mobileProjectUrl) {
            window.location.href = data.mobileProjectUrl;
        } else {
            // Fallback: construct URL from project name
            const projectName = encodeURIComponent(data.projectName);
            window.location.href = `/mobile/project/${projectName}`;
        }
    } catch (error) {
